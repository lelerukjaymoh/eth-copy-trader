...
const uniswapABI = JSON.parse(
    readFileSync("src/uniswap/uniswapABI.json", "utf8")
)

const smartContractABI = JSON.parse(
    readFileSync("src/uniswap/smartcontractABI.json", "utf8")
)

const smartContract = new web3.eth.Contract(smartContractABI, botParams.smartContractAddress)
...
const tokenAllowance = async (tokenAddress: string, walletAddress: string) => {
    try {
        ...
        return await tokenContract.methods.allowance(botParams.smartContractAddress, botParams.uniswapv2Router).call()
    } catch (error) {
...
export { toHex, currentNonce, uniswapABI, smartContract, tokenAllowance }